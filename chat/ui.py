...
class Message(ctk.CTkFrame):
    def __init__(self, master, text="", user="YOU"):
        ...
        self.text = ctk.CTkLabel(master=self,width=100, anchor="w", text=text, font=("sans", 12), fg_color="transparent")  
...
class MessageFrame(ctk.CTkScrollableFrame):
    ...
    def addMessage(self, text="", user="YOU"):
        ...
        if user == "YOU":
            message.grid(row=len(self.messages), column=0, sticky='se', padx=5, pady=(0,10))
        else : 
            message.grid(row=len(self.messages), column=0, sticky='sw', padx=5, pady=(0,10))
...
class APP(ctk.CTk):
    ...
    def __init__(self, usrnm="YOU"):
        super().__init__()
        self.username = bool(usrnm) if usrnm else "YOU"
        self.grid_rowconfigure(0, weight=1)
        self.grid_columnconfigure(0, weight=1)
    ...
    def sendMessage(self):
        ...
        self.mainFrame.inputFrame.entry.delete(0, ctk.END)
